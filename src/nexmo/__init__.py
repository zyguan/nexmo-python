    ...
    def __init__(
        ...
        *,
        ...
        if self.api_secret:
            ...
            creds.append(SecretCredentials(self.api_key, self.api_secret))
        if self.signature_secret:
            LOG.debug("Creating SignatureCredentials for: %s", self.api_key)
            creds.append(SignatureCredentials(self.api_key, self.signature_secret))
        if self.application_id:
            LOG.debug("Creating PrivateKeyCredentials for app: %s", self.application_id)
            creds.append(PrivateKeyCredentials(self.application_id, self.private_key))
